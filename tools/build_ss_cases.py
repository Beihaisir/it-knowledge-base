...
def build_cases() -> list[dict]:
    ...
    while i < len(msgs):
        ...
        if str(m.get("type")) == "1" and len(content) >= 12 and \
            ...
            if re.match(r"^(不是|下次|这个|那个|你们|我们|请|去|看|查|点|用|开|做|检查|登录|重启|找|联系|改|发|填|选|按|操作|在|进|到|输入|选择|确认|提交|解锁|恢复|申请|审核|处理|关闭|打开|设置|更换|重新|先|应该|可以|试|建议|好的|已经|可以了|解决了|谢谢|学习了|收到|嗯|好嘞|品管|我说的是|所以你们|找到这个)[，,：: 把]", _strip) or re.match(r"^(你|我)(们)?[，,：: ]", _strip) or (re.match(r"^不是", _strip) and not Q_END.search(content)) or re.match(r"^(好的|已经|谢谢|收到|可以了|解决了|学习了)", _strip) or (re.match(r"^(品管|我说的是|所以你们|找到这个|确实是|感谢|懂了)", _strip) and not Q_END.search(content)):
                i += 1
                continue
            answers = []
            j = i + 1
